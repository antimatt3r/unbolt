...
def decrypt_file(file_path, server_url, password, force, persist=True):
    ...
    if out_path.exists() and not force:
        return file_path, False, f"Output file '{out_path.name}' already exists. Use -f/--force to overwrite."
...
def main():
    ...
    parser.add_argument("files", nargs="+", help="One or more encrypted files to decrypt")
    ...
    # Determine URL
    env_vars = load_env()
    server_url = args.url or os.environ.get("UNBOLT_URL") or env_vars.get("UNBOLT_URL") or "http://localhost:8000"

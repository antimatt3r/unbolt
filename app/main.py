...
MAX_FILE_SIZE = int(
    os.environ.get("UNBOLT_MAX_FILE_SIZE", 100 * 1024 * 1024)
)  # default 100MB
RATE_LIMIT_REQUESTS = int(os.environ.get("UNBOLT_RATE_LIMIT_REQUESTS", 60))  # requests
RATE_LIMIT_WINDOW = int(
    os.environ.get("UNBOLT_RATE_LIMIT_WINDOW", 60)
)  # seconds (1 minute)
...
async def decrypt_file(
    ...
    if decrypted_bytes is None:
        raise HTTPException(
            status_code=422, detail="Failed to decrypt file with available passwords."
        )
